...
title_label = tk.Label(win, text="Student management system", font=("Arial", 25, "bold"), border=12, relief=tk.GROOVE)
...
def add_func():
    if rollno.get() == "" or name.get() == "" or class_var.get() == "":
        messagebox.showerror("Error!, Please fill all the fiends!")
    else:
        conn = pymysql.connect(host="localhost", user="root", passwd="", database="sms1")
        curr = conn.cursor()
        curr.execute("INSERT INTO data VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)", (rollno.get(), name.get(),section.get(), 
                                                                                     class_var.get(), contact.get(), fathersnm.get(), 
                                                                                     gender.get(), dob.get(), address.get()))
        conn.commit()
        conn.close()
        fetch_date()
...
def update_func():
    conn = pymysql.connect(host="localhost", user="root", password="", database="sms1")
    curr = conn.cursor()
    curr.execute("UPDATE data SET name=%s, section=%s, class=%s, contact=%s, fathersnm=%s, gender=%s, dob=%s, address=%s where rollno=%s", (name.get(),section.get(),class_var.get(), contact.get(), fathersnm.get(), gender.get(), dob.get(), address.get(),rollno.get()))
    conn.commit()
    conn.close()
    fetch_date()
    clear()
...
def delete_func():
    conn = pymysql.connect(host="localhost", user="root", password="", database="sms1")
    curr = conn.cursor()
    curr.execute("DELETE FROM data WHERE name=%s and section=%s and class=%s and contact=%s and fathersnm=%s and gender=%s and dob=%s and address=%s and rollno=%s", (name.get(),section.get(),class_var.get(), contact.get(), fathersnm.get(), gender.get(), dob.get(), address.get(),rollno.get()))
    conn.commit()
    conn.close()
    fetch_date()
    clear()
...
rollno_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15), textvariable=rollno)
...
name_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15),textvariable= name)
name_ent.grid(row=1, column=1, padx=2, pady=2)

class_lbl = tk.Label(detail_frame, text="Class", font= ("Arial",15), bg="lightgrey")
class_lbl.grid(row=2, column=0, padx=2, pady=2)

class_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15), textvariable= class_var)
...
section_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15), textvariable=section)
...
contact_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15), textvariable=contact)
...
fathersnm_ent = tk.Entry(detail_frame, bd=7, font=("Alrial", 15), textvariable=fathersnm)
...
address_ent = tk.Entry(detail_frame,bd=7, font=("Alrial", 15), textvariable=address)
...
gender_ent = ttk.Combobox(detail_frame, font=("Alrial", 15), textvariable=gender)
gender_ent['values'] = ("Male", "Female", "Orther")
gender_ent.grid(row=7, column=1, padx=2, pady=2)

dob_lbl = tk.Label(detail_frame, text="B.O.B", font= ("Arial",15), bg="lightgrey")
dob_lbl.grid(row=8, column=0, padx=2, pady=2)

dob_ent = tk.Entry(detail_frame,bd=7, font=("Alrial", 15), textvariable=dob)
...
search_lbl = tk.Label(search_frame, text="Search", font= ("Arial",15), bg="lightgrey")
...
showall_btn = tk.Button(search_frame, text="Search all", font=("Arial", 13), bd=9, width=14, bg="lightgrey")
showall_btn.grid(row=0, column=3, padx=12, pady=2)

#=========== DATABASE FRAME ===========#

main_frame = tk.Frame(data_frame, bg="lightgrey", bd=12, relief=tk.GROOVE)
main_frame.pack(fill=tk.BOTH, expand=True)

y_scroll = tk.Scrollbar(main_frame, orient=tk.VERTICAL)
x_scroll = tk.Scrollbar(main_frame, orient=tk.HORIZONTAL)

student_table = ttk.Treeview(main_frame, columns=("Roll No.", "Name", "Class", "Section", "Concat", "Father's Name", "Gender", "D.O.B", "Address"), yscrollcommand=y_scroll.set, xscrollcommand=x_scroll.set)
...
student_table.heading("Concat", text="Concat")
...
student_table.column("Concat",width=100)
